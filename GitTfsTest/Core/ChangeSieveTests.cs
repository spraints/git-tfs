    ...
    public class ChangeSieveTests
    {
        ...
        public abstract class BaseFixture
        {
            public BaseFixture()
            {
                ...
                // Make the GitTreeInformation have some initial modes.
                InitialTree.Stub(r => r.GetMode(null))
                    .Constraints(Is.Anything())
                    .Do(new Function<string, string>(path =>
                        InitialTreeModes.ContainsKey(path) ? InitialTreeModes[path] : null));
            }
            ...
            private IGitTreeInformation _initialTree;
            public virtual IGitTreeInformation InitialTree
            {
                get { return _initialTree ?? (_initialTree = BuildInitialTree()); }
            }
            protected virtual IGitTreeInformation BuildInitialTree()
            {
                return _mocks.StrictMock<IGitTreeInformation>();
            }

            private IDictionary<string, string> _initialTreeModes;
            public virtual IDictionary<string, string> InitialTreeModes
            {
                // Q: Is IGitTreeInformation case insensitive? Here we assume that it ignores case.
                get { return _initialTreeModes ?? (_initialTreeModes = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase)); }
            }
        ...
        public class WithExistingItems : Base<WithExistingItems.Fixture>
        {
            public class Fixture : BaseFixture
            {
                public Fixture()
                {
                    InitialTreeModes.Add("dir1/file1.exe", "100755");
                    InitialTreeModes.Add("dir1/file4.txt", "100644");
                    InitialTreeModes.Add("dir2/file2.txt", "111111");
            ...
            {
                // Path casing is now handled transparently by libgit2sharp.
                AssertChanges(Subject.GetChangesToApply(),
